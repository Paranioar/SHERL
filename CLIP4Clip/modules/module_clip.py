...
class Transformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, video_frame=-1):
        hidden_list = [x.permute(1, 0, 2)]

        for module in self.resblocks:
            x, video_frame = module((x, video_frame))
            hidden_list.append(x.permute(1, 0, 2))
...
class VisualTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, video_frame=-1):
        ...
        x, hidden_list = self.transformer(x, video_frame=video_frame)
...
class CLIP(nn.Module):
    ...
    def encode_image(self, image, return_hidden=False, video_frame=-1):
        ...
        with torch.no_grad():
            hidden, hidden_list = self.visual(image.type(self.dtype), video_frame=video_frame)
            hidden_list = torch.stack(hidden_list, dim=0)

            hidden = self.visual.ln_post(hidden) @ self.visual.proj     

        if return_hidden:
            return hidden[:, 0, :], hidden_list
        else:
            return hidden[:, 0, :]
    ...
    def encode_text(self, text, return_hidden=False):
        ...
        with torch.no_grad():
            ...
            x = x + pos_emd
            x = x.permute(1, 0, 2)  # NLD -> LND
            hidden, hidden_list = self.transformer(x)
            hidden = hidden.permute(1, 0, 2)  # LND -> NLD
            hidden_list = torch.stack(hidden_list, dim=0)

            hidden = self.ln_final(hidden).type(self.dtype) @ self.text_projection

        if return_hidden:
            return hidden[torch.arange(hidden.shape[0]), text.argmax(dim=-1)], hidden_list
        else:
            return hidden[torch.arange(hidden.shape[0]), text.argmax(dim=-1)]
