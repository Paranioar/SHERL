...
from lib.loss import ContrastiveLoss, diversity_loss
...
class VSEModel(object):
    ...
    def __init__(self, opt):
        ...
        self.img_enc = get_image_encoder(opt, opt.img_dim, opt.embed_size,
                                         precomp_enc_type=opt.precomp_enc_type,
                                         backbone_source=opt.backbone_source,
                                         backbone_path=opt.backbone_path,
                                         no_imgnorm=opt.no_imgnorm)
        self.txt_enc = get_text_encoder(opt, opt.embed_size, 
        ...
        self.optimizer = torch.optim.AdamW([
            {'params': self.txt_enc.need_train_params, 'lr': opt.learning_rate},
            {'params': self.img_enc.need_train_params, 'lr': opt.learning_rate},
    ...
    def make_data_parallel(self):
        ...
        logger.info('Dual encoders are data paralleled now.')
    ...
    def forward_loss(self, img_emb, cap_emb):
        ...
        self.logger.update('Le_rank', loss.data.item(), img_emb.size(0))
