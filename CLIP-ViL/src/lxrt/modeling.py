...
from .lora import LoRALinearController
...
class BertAttention(nn.Module):
    def __init__(self, config, ctx_dim=None):
        ...
        if config.lora_config is not None:
            self.use_lora = True
            self.query = LoRALinearController(ctx_dim, self.all_head_size, config=config.lora_config)
            self.value = LoRALinearController(ctx_dim, self.all_head_size, config=config.lora_config)
    ...
    def forward(self, hidden_states, context, attention_mask=None, task=None):
        if self.use_lora:
            self.query.forward = partial(self.query.forward, task=task)
            self.value.forward = partial(self.value.forward, task=task)
...
class BertCrossattLayer(nn.Module):
    ...
    def forward(self, input_tensor, ctx_tensor, ctx_att_mask=None, task=None):
        output = self.att(input_tensor, ctx_tensor, ctx_att_mask, task)
...
class BertSelfattLayer(nn.Module):
    ...
    def forward(self, input_tensor, attention_mask, task=None):
        ...
        self_output = self.self(input_tensor, input_tensor, attention_mask, task)
...
class LXRTEncoder(nn.Module):
    def __init__(self, config):
        ...
        self.train_side_transformer = True if config.side_config is not None else False

        self.detach_visual_projection = config.side_config.detach_visual_projection if config.side_config is not None else False
        
        layers = [BertLayer(config) for _ in range(self.num_l_layers)]
        self.layer = nn.ModuleList(layers)
        print("\n\nVisualBERT style: {} layers".format(len(self.layer)))

        if config.side_config is not None:
            self.side_block = Sherl_Block(config.hidden_size, 
                                          config.hidden_size // config.side_config.reduction_factor,
                                          self.num_l_layers)
    ...
    def forward(self, lang_feats, lang_attention_mask,
        ...
        if self.train_side_transformer:
            all_hidden_features = []
            all_hidden_features.append(joint_feats)

        for idx, layer_module in enumerate(self.layer):

            if idx < self.num_l_layers - 1:
                ...
                if self.train_side_transformer:
                    joint_feats = self.side_block(all_hidden_features, joint_feats)
...
class LXRTFeatureExtraction(BertPreTrainedModel):
    ...
    """
    def __init__(self, config, mode='lxr', adapter_config=None, vis_adapter_config=None, side_config=None, lora_config=None):
        """
        ...
        config.side_config = side_config
