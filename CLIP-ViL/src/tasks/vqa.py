...
class VQA(TrainerBase):
    ...
    def create_side_network_initial_weights(self):
        ...
        if "fisher" in self.args.load_side_pretrained_weights or "bert" in self.args.load_side_pretrained_weights:
            ...
            model = VQAModel(self.train_tuple.dataset.num_answers) # ad-hoc writing
            ...
            # prepare the dataset
            train_dataset = VQATorchDataset(VQADataset(self.args.train)) # ad-hoc writing
            ...
            if "fisher" in self.args.load_side_pretrained_weights:
                # ad-hoc writing
                importance_measure = compute_fisher(model, "vqa", train_dataset, data_collator, num_samples=self.args.samples_for_fisher)
            else:
