...
from seq2seq.third_party.models.t5.modeling_side_outputs import SideBaseModelOutputWithPastAndCrossAttentions, SideSeq2SeqLMOutput, SideBaseModelOutput
...
class NormLinear(nn.Module):
    def __init__(self, in_dim, out_dim, config):
        ...
        self.norm = T5LayerNorm(in_dim, eps=config.layer_norm_epsilon, adapter_config=None)
...
class T5Stack(T5PreTrainedModel):
    def __init__(self, config, embed_tokens=None, adapter_config=None, prefix_emb=None):
        ...
        side_config.d_ff = side_config.d_ff // self.adapter_config.task_reduction_factor
        ...
        side_layers = side_config.decoder_side_layers if self.is_decoder else side_config.encoder_side_layers

        if side_layers is None:
            # add every layer
            side_layers = list(range(config.num_layers))

        self.first_available_layer = side_layers[0] # the first layer has not-None module, will be used to let it has relative attention bias

        self.side_block = nn.ModuleList(
            [T5Block(self.per_layer_config(side_config, i, self.adapter_config, self.is_decoder),
                     has_relative_attention_bias=bool(i == self.first_available_layer),
        ...
        if self.use_gate == "learnable":
            ...
            self.side_gate_params = nn.ParameterList(
                [nn.Parameter(torch.ones(1) * config.gate_alpha) 
                for i in range(config.num_layers)]
            )

            self.gate_T = config.gate_T
        elif "schedule" in self.use_gate or self.use_gate == "one":
            for i in range(config.num_layers):
                self.register_buffer('side_gate_' + str(i), torch.ones(1))
    ...
    def parallelize(self, device_map=None):
        ...
        for k, v in self.device_map.items():
            for layer in v:
                cuda_device = "cuda:" + str(k)
                self.side_downsamples[layer] = self.side_downsamples[layer].to(cuda_device)

        # Set embed_tokens to first layer
        self.embed_tokens = self.embed_tokens.to(self.first_device)
        # Set final layer norm to last device
        self.final_layer_norm = self.final_layer_norm.to(self.last_device)
        self.final_side_layer_norm = self.final_side_layer_norm.to(self.last_device)
    ...
    def forward(
        ...
        for i, (layer_module, past_key_value) in enumerate(zip(self.block, past_key_values)):
            ...
            if self.model_parallel:
                ...
                if backbone_hidden_state is not None:
                    backbone_hidden_state = backbone_hidden_state.to(hidden_states.device)
                ...
                if side_encoder_hidden_states is not None:
                    side_encoder_hidden_states = side_encoder_hidden_states.to(hidden_states.device)
                if encoder_extended_attention_mask is not None:
                    encoder_extended_attention_mask = encoder_extended_attention_mask.to(hidden_states.device)
                if encoder_decoder_position_bias is not None:
                    encoder_decoder_position_bias = encoder_decoder_position_bias.to(hidden_states.device)
                if side_encoder_decoder_position_bias is not None:
                    side_encoder_decoder_position_bias = side_encoder_decoder_position_bias.to(hidden_states.device)
                if layer_head_mask is not None:
            ...
            if getattr(self.config, "gradient_checkpointing", False) and self.training:
                ...
                side_layer_outputs = checkpoint(
                    create_custom_forward(side_layer_module),
                    side_hidden_states,
                    extended_attention_mask,
                    position_bias,
                    side_encoder_hidden_states,
                    encoder_extended_attention_mask,
                    encoder_decoder_position_bias,
                    layer_head_mask,
                    cross_attn_layer_head_mask,
                ...
                if side_layer_module is not None:
                    ...
                    if output_attentions:
                        all_side_attentions = all_side_attentions + (None,)
                        if self.is_decoder:
                            all_side_cross_attentions = all_side_cross_attentions + (None,)
            ...
            if self.model_parallel:
                for k, v in self.device_map.items():
                    if i == v[-1] and "cuda:" + str(k) != self.last_device:
                        ...
                        side_hidden_states = side_hidden_states.to("cuda:" + str(k + 1))
        ...
        return SideBaseModelOutputWithPastAndCrossAttentions(
            ...
            last_side_hidden_state=side_hidden_states,
...
class Adapter(nn.Module):
    ...
    def forward(self, x):
        ...
        x = (self.fc2(x) + x) * torch.sigmoid(self.fc3(x))
...
class MTSAdapter(nn.Module):
    def __init__(self, input_dim, hidden_dim, num_layer):
        super().__init__()

        self.num_layer = num_layer
        self.guide_block = Guidance(hidden_dim)

        self.hor_block = nn.ModuleList(
            [Adapter(input_dim, hidden_dim, hidden_dim) for _ in range(num_layer)])
    ...
    def forward(self, all_features_list, last_feature):
        ...
        out_x = self.fc_out(torch.relu(guide_x)) 
        gate = torch.tanh(self.gate_params / self.gate_T)

        return out_x + gate * last_feature
    

class T5StackEnc(T5PreTrainedModel):
    def __init__(self, config, embed_tokens=None, adapter_config=None, prefix_emb=None):
        super().__init__(config)
        ...
        # ----------- side block ------------ #
        self.side_layers = config.decoder_side_layers if self.is_decoder else config.encoder_side_layers

        side_config = copy.deepcopy(config)
        side_config.d_ff = config.d_ff // self.adapter_config.task_reduction_factor
        side_config.d_kv = config.d_kv // self.adapter_config.task_reduction_factor
        side_config.d_model = config.d_model // self.adapter_config.task_reduction_factor

        self.side_mtsa_block = MTSAdapter(config.d_model, side_config.d_model, len(self.side_layers) + 1)
        self.final_side_layer_norm = T5LayerNorm(config.d_model, eps=side_config.layer_norm_epsilon)
        self.dropout = nn.Dropout(config.dropout_rate)
        # ----------- side block ------------ #
    ...
    def parallelize(self, device_map=None):
        ...
        for k, v in self.device_map.items():
            for layer in v:
                cuda_device = "cuda:" + str(k)
                self.side_hor_block[layer] = self.side_hor_block[layer].to(cuda_device)
    ...
    def linear_attention(self, query, key, value, mask=None):
        cross_weights = torch.matmul(query, key.permute(0, 2, 1))

        if mask is not None:
            cross_weights = cross_weights * mask

        cross_weights = l1norm(torch.relu(cross_weights), dim=-1) 
        wcontext = torch.matmul(cross_weights, value)
    ...
    def forward(
        ...
        hidden_states = self.dropout(inputs_embeds)
        all_hidden_features = [hidden_states]
        ...
        for i, (layer_module, past_key_value) in enumerate(zip(self.block, past_key_values)):
            ...
            if getattr(self.config, "gradient_checkpointing", False) and self.training:
                ...
                if i in self.side_layers:
                    all_hidden_features.append(layer_outputs[0])

                if i == self.side_layers[-1]:
                    side_hidden_states = self.side_mtsa_block(all_hidden_features, layer_outputs[0])
        ...
        # ---------------------------------- #
        hidden_states = self.dropout(self.final_layer_norm(layer_outputs[0]))
        side_hidden_states = self.dropout(self.final_side_layer_norm(side_hidden_states))
        # ---------------------------------- #
...
class T5StackDec(T5PreTrainedModel):
    def __init__(self, config, embed_tokens=None, adapter_config=None, prefix_emb=None):
        super().__init__(config)
        ...
        # ----------- side block ------------ #
        side_layers = config.decoder_side_layers if self.is_decoder else config.encoder_side_layers
        ...
        assert side_layers is not None

        self.insert_layer = config.num_layers - 1
        self.side_layers = []
        for i in side_layers:
            if i < self.insert_layer:
                self.side_layers.append(i)
    ...
    def forward(
        ...
        for i, (layer_module, past_key_value) in enumerate(zip(self.block, past_key_values)):
            ...
            if getattr(self.config, "gradient_checkpointing", False) and self.training:
                ...
                if i < self.insert_layer:
                    ...
                    if i == self.insert_layer:
                        hidden_states = self.side_mtsa_block(all_hidden_features, layer_outputs[0])
        ...
        # ---------------------------------- #
        hidden_states = self.dropout(self.final_layer_norm(layer_outputs[0]))
        side_hidden_states = self.dropout(self.final_side_layer_norm(layer_outputs[0]))
        # ---------------------------------- #
...
class T5ForConditionalGeneration(SideGenerationMixin, T5PreTrainedModel):
    ...
    def __init__(self, config, adapter_config=None, lora_config=None):
        ...
        encoder_config.use_cache = False
        encoder_config.is_encoder_decoder = False
        self.encoder = T5StackEnc(encoder_config, self.shared, adapter_config=adapter_config, prefix_emb=self.prefix_shared)
        ...
        self.decoder = T5StackDec(decoder_config, self.shared, adapter_config=adapter_config, prefix_emb=self.prefix_shared)
        ...
        if config.merge_last:
            self.merge_last = True
            self.gate_T = config.gate_T
            self.side_gate_params_merge_last = nn.Parameter(torch.ones(1) * config.gate_alpha) 
    ...
    def forward(
        ...
        if self.prefix_tuning:
            if attention_mask is not None:
                attention_mask = torch.cat([torch.ones((attention_mask.shape[0], self.prefix_dim)).to(attention_mask.device), attention_mask], dim=1)
        ...
        if encoder_outputs is None:
            ...
            encoder_outputs = SideBaseModelOutput(
                ...
                hidden_states=encoder_outputs[1] if len(encoder_outputs) > 1 else None,
                attentions=encoder_outputs[2] if len(encoder_outputs) > 2 else None,
                last_side_hidden_state=encoder_outputs.last_side_hidden_state,
        ...
        backbone_sequence_ouptut = decoder_outputs.last_hidden_state

        if self.merge_last:
            side_gate_param = self.side_gate_params_merge_last
            gate = torch.sigmoid(side_gate_param / self.gate_T)
            sequence_output = gate * sequence_output + (1 - gate) * decoder_outputs.last_hidden_state # add the information from the backbone network
        ...
        if self.model_parallel:
            ...
            backbone_sequence_ouptut = backbone_sequence_ouptut.to(self.lm_head.weight.device)
        ...
        if self.config.tie_word_embeddings:
            ...
            backbone_sequence_ouptut = backbone_sequence_ouptut * (self.model_dim ** -0.5)
    ...
    def _update_model_kwargs_for_generation(
        ...
        if "side_past_key_values" in outputs:
            model_kwargs["side_past_key_values"] = outputs.side_past_key_values
