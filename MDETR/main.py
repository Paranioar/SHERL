...
def main(args):
    ...
    param_dicts = [
        {
            "params": [p for n, p in model_without_ddp.named_parameters() if p.requires_grad and "side" in n]
        },
        {
            "params": [p for n, p in model_without_ddp.named_parameters() if p.requires_grad and "side" not in n],
            "lr": args.lr * 0.1,
        }
    ]
    # --------------------------------------------------------
    ...
    if args.eval:
        ...
        if torch.cuda.is_available():
            ...
            print("Memory utilization: %.3f GB" % peak_memory)
        print("Time utilization: %.3f s" % (time.time() - end) )
        return
